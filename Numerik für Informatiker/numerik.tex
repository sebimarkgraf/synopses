...
\begin{document}
	...
	\textbf{Idee}: Stelle Iterationsverfahren in Matrizenschreibweise dar.\\\\
	\textbf{Vorkonditionierung}: Um die Kondition der Matrix A zu verbessern, wird ein Vorkonditionierer $B \in \realnumbers^{N\times N}$ gewählt. Hierzu zerteilen wir A in eine strikte untere linke Dreiecksmatrix L, eine strikte obere rechte Dreiecksmatrix R und eine Diagonalmatrix D mit $A = L + D + R$. Damit ist der Vorkonditionierer für das Jacobi-Verfahren $B = D^{-1}$ und für das Gauß-Seidel-Verfahren $B = (L + D)^{-1}$. Für die vorkonditionierte Aufgabe gilt damit:
	\begin{center}
		$Ax = b \Leftrightarrow BAx = Bb$
	\end{center}
	\textbf{Iterationsmatrizen}: TODO (nicht im Skript dokumentiert)\\\\
